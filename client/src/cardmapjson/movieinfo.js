...
function Movieinfo() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:3001/getssinglemoviebyid/${animeId}`)
      .then((res) => setAnime(res.data))
      .catch((error) => console.error("Error fetching anime details:", error));
  }, [animeId]);

  if (!anime) {
    return <h1 style={{ textAlign: "center" }}>No movie data available</h1>;
  }
  return (
    <div>
      <Navbar />
        ...
        <div className="anime-banner">
          ...
          <div className="anime-content">
            ...
            <div className="anime-info">
              ...
              <p><strong>Duration:</strong> {`${anime.duration} min`}</p><br/>
              ...
              <div className="anime-actions">
                {anime.watchLink && (
                  <a href={anime.watchLink} target="_blank" rel="noopener noreferrer">
                    <button className="play-button">▶ Watch Movie</button>
                  </a>
                )}
                <button onClick={() => navigate("/moviepage")} className="list-button">
                  Movie page
                </button>
              </div>
            </div>
